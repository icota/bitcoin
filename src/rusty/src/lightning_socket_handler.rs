...
use core_lightning::EventHandler;
...
impl Connection {
    pub fn schedule_read(peer_manager: Arc<peer_handler::PeerManager<SocketDescriptor>>, us: Arc<Mutex<Self>>) {
        let us_ref = us.clone();
        let us_close_ref = us.clone();
        let peer_manager_ref = peer_manager.clone();

        let mut stream = us.lock().unwrap().stream.try_clone().unwrap();

        println!("Reading from {}", stream.peer_addr().unwrap().ip().to_string());
        ...
        if len > 0 {
            let mut pending_read = [0; 512];

            let read = stream.read(&mut pending_read).unwrap();
//            loop {
//                match stream.read_to_end(&mut pending_read) {
//                    Ok(_) => break,
//                    Err(e) => panic!("encountered IO error: {}", e),
//                };
//            };
            println!("Read {} bytes!", pending_read.len());
            println!("Read read {} bytes!", read);
            //println!("bytes: {:?}", pending_read);

            match peer_manager.read_event(&mut SocketDescriptor::new(us_ref.clone(), peer_manager.clone()), pending_read[..read].to_vec()) {
                Ok(pause_read) => {
                    if pause_read {
            ...
            us_ref.lock().unwrap().event_notify.process_events();
...
fn new(event_notify: Arc<EventHandler>, stream: TcpStream) -> Arc<Mutex<Self>> {
    let us = Arc::new(Mutex::new(Self { event_notify, pending_read: Vec::new(), read_paused: false, need_disconnect: true, id: ID_COUNTER.fetch_add(1, Ordering::AcqRel), stream, read_scheduled: true }));
    us
}
    ...
    pub fn setup_inbound(peer_manager: Arc<peer_handler::PeerManager<SocketDescriptor>>, event_notify: Arc<EventHandler>, stream: TcpStream) {
        let us = Self::new(event_notify, stream);

        if let Ok(_) = peer_manager.new_inbound_connection(SocketDescriptor::new(us.clone(), peer_manager.clone())) {
            //Self::schedule_read(peer_manager, us);
        }
    }
    ...
    /// You should poll the Receive end of event_notify and call get_and_clear_pending_events() on
    /// ChannelManager and ChannelMonitor objects.
    pub fn setup_outbound(peer_manager: Arc<peer_handler::PeerManager<SocketDescriptor>>, event_notify: Arc<EventHandler>, their_node_id: PublicKey, stream: TcpStream) {
        let us = Self::new(event_notify, stream.try_clone().unwrap());
        ...
        if let Ok(initial_send) = peer_manager.new_outbound_connection(their_node_id, SocketDescriptor::new(us.clone(), peer_manager.clone())) {
            if SocketDescriptor::new(us.clone(), peer_manager.clone()).send_data(&initial_send, true) == initial_send.len() {
                Self::schedule_read(peer_manager.clone(), us.clone());
                Self::schedule_read(peer_manager.clone(), us.clone());
            } else {
...
impl peer_handler::SocketDescriptor for SocketDescriptor {
    fn send_data(&mut self, data: &[u8], resume_read: bool) -> usize {
        // TODO: reimplement w/o tokio or future
        // this should return 50 on the first go

        println!("Writing to: {}", self.conn.lock().unwrap().stream.peer_addr().unwrap().ip().to_string());
        //can't read?
        let bytes_written = self.conn.lock().unwrap().stream.write(data).unwrap();
        // TODO: fix how this blocks everything, figure out how to schedule after writes
        //Connection::schedule_read(self.peer_manager.clone(), self.conn.clone(), self.conn.lock().unwrap().stream.try_clone().unwrap());
