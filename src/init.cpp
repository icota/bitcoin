...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.GetBoolArg("-lightning", lightning::DEFAULT_LIGHTNING)) {
        std::string lightning_sub_ver = FormatSubVersion(RUSTY_LIGHTNING_CLIENT_NAME, CLIENT_VERSION, uacomments);
        unsigned short bind_port = (unsigned short)(gArgs.GetArg("-lightningport", lightning::DEFAULT_LIGHTNING_PORT));
        lightning_node = lightning::init_node(node.connman.get(), GetDataDir().c_str(), lightning_sub_ver.c_str());
        if (bind_port) {
            lightning::listen_incoming(lightning_node, bind_port);
        }
        //lightning::connect_peer(lightning_node, "test");
    }
#endif
